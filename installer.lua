...
print("A.E.O.N installer")
term.setTextColor(colors.white)
print(("Source: %s/%s [%s]"):format(REPO_OWNER, REPO_NAME, branch))

local manifestBody, manifestErr = fetch(joinUrl(rawBase, "manifest.lua"))
if not manifestBody then
  error(manifestErr, 0)
end

local manifest, parseErr = loadTableFromString(manifestBody, "@manifest.lua")
...
print(("AEON %s installed"):format(tostring(manifest.version or "unknown")))
